...
2. 「録音開始」ボタンで会議を録音（5分ごとに自動でセグメント分割される）
...
### Step 2: Netlify Functions化 ← 現在地

`transcribe` / `generate-report` の2つの Functions を実装し、APIキーを環境変数に移す。ブラウザからキーを消す。長尺音声対応として、5分ごとのセグメント分割＋並列文字起こしも実装。

### Step 3: 2画面UI＋Deploy API統合

メイン画面（録音）＋結果画面（URL表示／エラー表示）。Netlify Deploy API でレポートを固定URLに配信し、家族にURLを共有できるようにする。エラー時の録音ダウンロード導線を含む。
